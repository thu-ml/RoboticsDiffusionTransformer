...
def process_step(step: dict) -> dict:
    ...
    eef_ang = tf.gather(action[3:6], [2, 1, 0])
    ...
    arm_joint_state = state['joint_state']
    arm_joint_pos = arm_joint_state[:7]
    # We do not use the velocity since it is inacurate
    # arm_joint_vel = arm_joint_state[7:14]
    # Robot joint trajectory, consists of [7x robot joint angles, 7x robot joint velocity, 7x robot joint acceralation].
    # arm_joint_traj = state['joint_trajectory']
    # Robot end effector pose, consists of [3x EEF position, 3x EEF orientation yaw/pitch/roll].
    eef = state["end_effector_pose"]
    eef_pos = eef[:3]
    eef_ang = tf.gather(eef[3:6], [2, 1, 0])
    ...
    state['arm_concat'] = tf.concat([arm_joint_pos, eef_pos, eef_ang], axis=0)
